...
contains
        ...
        do i = multi_domain%subdomains(n, m)%is, multi_domain%subdomains(n, m)%ie
          do j = multi_domain%subdomains(n, m)%js, multi_domain%subdomains(n, m)%je
            if (one_hill.eq.1) then
              ...
              dy = (multi_domain%subdomains(n, m)%y(j) - 0.9_8 * (maxval(multi_domain%global_domain%y) - minval(multi_domain%global_domain%y)) / 2.0_8) / multi_domain%global_domain%y(multi_domain%global_domain%je)
  ...
  !of characteristic depth scale_h and characteristic scale scale_sigma.
    ...
    do n = 1, multi_domain%num_sub_x
      do m = 1, multi_domain%num_sub_y
        do i = multi_domain%subdomains(n, m)%is, multi_domain%subdomains(n, m)%ie
          do j = multi_domain%subdomains(n, m)%js, multi_domain%subdomains(n, m)%je
            dx = (multi_domain%subdomains(n, m)%x(i) - 0.5 * (maxval(multi_domain%global_domain%x) - minval(multi_domain%global_domain%x)) / 2.0_8)
            dy = (multi_domain%subdomains(n, m)%y(j) - (maxval(multi_domain%global_domain%y) - minval(multi_domain%global_domain%y)) / 2.0_8)
  ...
  subroutine set_swm_baratropic_instability(state, multi_domain, h_mean, u0)
    ...
    integer(kind=8) :: i, j, n, m, k, n_sub, m_sub
    real(kind=8)    :: c, lx, ly
    type(multi_grid_field_t) :: d1, d2, h_pret
    real(kind=8), allocatable :: Y_func(:)
    ...
    allocate(Y_func(1:100000))
    do k = 1,1e4
      Y_func(k) = (ly / 1e4_8) * (k - 1)
    end do
      ...
      do m = 1, multi_domain%num_sub_y
        do i = multi_domain%subdomains(n, m)%is, multi_domain%subdomains(n, m)%ie
          do j = multi_domain%subdomains(n, m)%js, multi_domain%subdomains(n, m)%je
            ...
            state%u%subfields(n, m)%f(i, j) = u0 * (sin(2.0_8 * pi * multi_domain%subdomains(n, m)%y(j) / ly) ** 81.0_8)
            state%h%subfields(n, m)%f(i, j) = h_mean !phone field for h

            do k = 1,1e4
              if (multi_domain%subdomains(n, m)%y(j) >= Y_func(k)) then
                state%h%subfields(n, m)%f(i, j) = state%h%subfields(n, m)%f(i, j) - pcori / Earth_grav * (u0 * (sin(2.0_8 * pi * Y_func(k) / ly) ** 81.0_8)) * (ly / 1e4_8)
              else
                exit;
              end if
            end do
