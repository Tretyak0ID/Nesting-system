        ...
        call layer_re%create_similar(interp_layer_ls)
          ...
          if (diff_method == 'sbp21_2') then
            if (2 * domains%subdomains(n - 1, m)%ny == domains%subdomains(n, m)%ny) then
              call interp_MC2order_2to1ratio(layer_ls, interp_layer_ls, 'coarse2fine')
              call interp_MC2order_2to1ratio(layer_le, interp_layer_le, 'coarse2fine')
              call interp_MC2order_2to1ratio(layer_rs, interp_layer_rs, 'fine2coarse')
              call interp_MC2order_2to1ratio(layer_re, interp_layer_re, 'fine2coarse')
            else if (domains%subdomains(n - 1, m)%ny == 2 * domains%subdomains(n, m)%ny) then
              call interp_MC2order_2to1ratio(layer_ls, interp_layer_ls, 'fine2coarse')
              call interp_MC2order_2to1ratio(layer_le, interp_layer_le, 'fine2coarse')
              call interp_MC2order_2to1ratio(layer_rs, interp_layer_rs, 'coarse2fine')
              call interp_MC2order_2to1ratio(layer_re, interp_layer_re, 'coarse2fine')
            end if
          else if (diff_method == 'sbp42_2') then
            if (2 * domains%subdomains(n - 1, m)%ny == domains%subdomains(n, m)%ny) then
