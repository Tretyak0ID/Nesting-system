...
implicit none
  ...
  call curl%init_on_domain(domain)
  call op%init(diff_op, diff_op)

  call create_timescheme(timescheme, state, 'rk4')
  call swm_geostrophic_cyclone(state, domain, H_MEAN, 22.0_8 * 10.0_8 ** (-3.0_8), 1000000.0_8)

  do t = 0, Nt
    if (mod(t, 100) == 0) print *, 'step: ',  t
    call calc_curl(curl, state%u, state%v, domain, diff_op, diff_op)
    call write_field(state%h, domain, './data/test3h.dat', t + 1)
    call write_field(curl, domain, './data/test3curl.dat', t + 1)
