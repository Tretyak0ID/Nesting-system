program test_3_geostriphic_cyclone
...
implicit none
  ...
  call swm_geostrophic_cyclone(state, multi_domain, H_MEAN, 22.0_8 * 10.0_8 ** (-3.0_8), 1000000.0_8)

  do t = 0, Nt
    if (mod(t, 100) == 0) print *, 'step: ',  t
    call calc_curl(curl, state%u, state%v, multi_domain, sbp21, sbp21)
    ...
    call write_field(curl%subfields(1, 1), multi_domain%subdomains(1, 1), './data/test3curl_left.dat', t + 1)
    call write_field(curl%subfields(2, 1), multi_domain%subdomains(2, 1), './data/test3curl_right.dat', t + 1)
    call timescheme%step(state, op, multi_domain, dt)
  end do

end program test_3_geostriphic_cyclone
