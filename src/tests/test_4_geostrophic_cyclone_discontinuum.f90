...
implicit none
  ...
  !domain and dynamic operator init (add forcing)
  sbp21%name    = 'sbp21_1'
  sbp42%name    = 'sbp42_1'
  ...
  call swm_geostrophic_cyclone(state, multi_domain, H_MEAN, scale_h, scale_sigma, h0, u0, v0, 'const_discontinuum')
  ...
  do t = 0, Nt
    ...
    if (num_sub_x > 1) then 
      ...
      if (mod(t, t_step_rec) == 0) call write_field(curl%subfields(2, 1), multi_domain%subdomains(2, 1), './data/test4curl.dat', t / t_step_rec + 1)
    end if
