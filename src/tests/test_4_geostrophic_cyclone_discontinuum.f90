...
implicit none
  ...
  do t = 0, Nt
    ...
    if (num_sub_x > 1) then 
      if (mod(t, t_step_rec) == 0) call write_field(curl%subfields(1, 1), multi_domain%subdomains(1, 1), './data/test4curl_left.dat', t / t_step_rec + 1)
      if (mod(t, t_step_rec) == 0) call write_field(curl%subfields(2, 1), multi_domain%subdomains(2, 1), './data/test4curl_right.dat', t / t_step_rec + 1)
    else
  ...
  print *, 'test_4_geostrophic_cyclone_discontinuum multiscale successfully completed'
  ...
  !initial conditions init
  call set_swm_geostrophic_cyclone(state_left, multi_domain_left, H_MEAN, scale_h, scale_sigma, h0, u0, v0, 'const_discontinuum')
  ...
  do t = 0, Nt
    ...
    if (num_sub_x > 1) then 
      if (mod(t, t_step_rec) == 0) call write_field(curl_left%subfields(1, 1), multi_domain_left%subdomains(1, 1), './data/test4lcurl_left.dat', t / t_step_rec + 1)
      if (mod(t, t_step_rec) == 0) call write_field(curl_left%subfields(2, 1), multi_domain_left%subdomains(2, 1), './data/test4lcurl_right.dat', t / t_step_rec + 1)
    else
      if (mod(t, t_step_rec) == 0) call write_field(curl_left%subfields(1, 1), multi_domain_left%subdomains(1, 1), './data/test4curl.dat', t / t_step_rec + 1)
    end if
  ...
  print *, 'test_4_geostrophic_cyclone_discontinuum left scale successfully completed'
  ...
  !initial conditions init
  call set_swm_geostrophic_cyclone(state_right, multi_domain_right, H_MEAN, scale_h, scale_sigma, h0, u0, v0, 'const_discontinuum')
  ...
  do t = 0, Nt
    ...
    if (num_sub_x > 1) then 
      if (mod(t, t_step_rec) == 0) call write_field(curl_right%subfields(1, 1), multi_domain_right%subdomains(1, 1), './data/test4rcurl_left.dat', t / t_step_rec + 1)
      if (mod(t, t_step_rec) == 0) call write_field(curl_right%subfields(2, 1), multi_domain_right%subdomains(2, 1), './data/test4rcurl_right.dat', t / t_step_rec + 1)
    else
      if (mod(t, t_step_rec) == 0) call write_field(curl_right%subfields(1, 1), multi_domain_right%subdomains(1, 1), './data/test4curl.dat', t / t_step_rec + 1)
    end if
  ...
  print *, 'test_4_geostrophic_cyclone_discontinuum right scale successfully completed'
